addEventListener("fetch", event => {
  ...
  async function handleRequest(request) {
    ...
    const address = url.pathname.replace('/balances/vesting/','').replace('/','')
  
    if(!address || !validate_cheqd_address(address)) return new Response("No address specified or wrong address format.")
  
    return new Response( await calculate_vesting_coins( address ) )
  }
  
  async function calculate_vesting_coins(address) {
    ...
    const {
      ...
      ratio: Number(( time_vested_in_days - time_elapsed_in_days )/ time_vested_in_days)
    }
