...
export class NodeApi {
    ...
    constructor(public readonly base_rpc_api_url: string) {
    }

    async bank_get_total_supply_ncheq(): Promise<number> {
        let resp = await fetch(`${this.base_rpc_api_url}/cosmos/bank/v1beta1/supply/ncheq`);
    ...
    async auth_get_account(address: string): Promise<Account> {
        let resp = await fetch(`${this.base_rpc_api_url}/cosmos/auth/v1beta1/accounts/${address}`);
